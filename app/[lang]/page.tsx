...
export default async function Home({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return (
    <main className="h-screen max-h-screen bg-[#050505] text-zinc-300 flex flex-col justify-between selection:bg-red-950 selection:text-red-300 antialiased relative overflow-hidden">
      ...
      {/* 2. ТВОИ ОРИГИНАЛЬНЫЕ АНИМАЦИИ И ЭФФЕКТЫ (СОХРАНЕНЫ ПОЛНОСТЬЮ) */}
      <style dangerouslySetInnerHTML={{__html: `
        @keyframes subtle-zoom { 0% { transform: scale(1.02); } 100% { transform: scale(1.07); } }
        @keyframes lantern-glow-fast { 0% { opacity: 0.15; transform: scale(0.93); filter: blur(50px); } 100% { opacity: 0.75; transform: scale(1.07); filter: blur(65px); } }
        @keyframes window-glow { 0% { opacity: 0.1; transform: scale(0.5); filter: blur(40px); } 100% { opacity: 0.5; transform: scale(1.04); filter: blur(55px); } }
        .animate-bg-fog { animation: subtle-zoom 35s infinite alternate ease-in-out !important; }
        .glow-lantern-fast { animation: lantern-glow-fast 3s infinite alternate ease-in-out !important; }
        .glow-windows-white { animation: window-glow 5s infinite alternate ease-in-out !important; }
      ...
      <div className="absolute top-0 left-0 w-full h-screen z-0 pointer-events-none overflow-hidden">
        ...
        <Image src="/hero-bg.png" alt="Emberhome World" fill priority className="object-cover object-center transform animate-bg-fog opacity-90" />
        <div className="absolute right-[4%] bottom-[42%] w-72 h-72 rounded-full z-10 pointer-events-none mix-blend-screen glow-lantern-fast" style={{ backgroundColor: 'rgba(245, 158, 11, 0.28)' }} />
        <div className="absolute left-[15%] top-[5%] w-80 h-48 rounded-full z-10 pointer-events-none mix-blend-screen glow-windows-white" style={{ backgroundColor: 'rgba(255, 255, 255, 0.5)' }} />
      </div>

      {/* ШАПКА САЙТА */}
      <header className="w-full max-w-7xl mx-auto px-6 py-6 flex justify-between items-center relative z-40 border-b border-zinc-900/40">
...
<section className="w-full max-w-7xl mx-auto px-6 pb-12 relative z-20 grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
  {currentTranslations.cards?.map((card: any, i: number) => {
    // Исправленная и полностью локализованная база лора на 10 языках
    const backTexts: Record<number, Record<string, { title: string; desc: string }>> = {
      ...
      2: {
        ...
        es: { title: "CONSTRUYE", desc: "Construye muros, torres de defensa y otras estructuras." },
      ...
      3: {
        ...
        de: { title: "VERTEIDIGE", desc: "Verteidige dich nicht gegen die Dunkelheit, verteidige dich gegen diejenigen, die nach Licht dürsten." },
  ...
  {/* ДИНАМИЧЕСКИЙ БЛОК ЖИВЫХ НОВОСТЕЙ ИЗ SANITY (Оставляем без изменений) */}
<div className="md:col-span-3 lg:col-span-2.6 glass-panel p-5 rounded-sm h-56 flex flex-col justify-between w-full hidden md:flex">
        ...
        liveNews.map((newsItem: any, index: number) => {
          ...
          return (
            <Link key={index} href={newsUrl} className="group block cursor-pointer no-underline">
              ...
              <h4 className="text-sm text-zinc-400 group-hover:text-amber-500 transition truncate font-light">
                {newsItem.title}
              </h4>
            </Link>
          );
